...
// MODIFIED:    
// DESCRIPTION: Synthetic Control Method (SCM) for Net Migration Rate
// ============================================================================
...
foreach i in 1 2 3 4 5 10 15 {
    gen L`i'_mig = L`i'.migration_rate
}

// --- b. Lags of Controls ---
foreach v in lrgdppc bankcrisis institutions {
    gen L1_`v' = L.`v'
}
...
allsynth migration_rate `preds',
	...
	keep(${data_clean}/mig_all.dta, replace) ;
...
allsynth migration_rate `preds',
    ...
    pvalues(rmspe variance)
	bcorrect(merge)
	keep(${data_clean}/mig_left.dta, replace) ;
...
allsynth migration_rate `preds',
    ...
    keep(${data_clean}/mig_right.dta, replace) ;
#delimit cr
