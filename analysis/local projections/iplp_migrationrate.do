...
foreach r in bankcrisis currcrisis debtcrisis dmig wdmig institutions war gini unemployrate conflicts advanced pop_jul global medial inflation debtgdp {
qui tssmooth ma `r'_wma = `r',  weights(5 4 3 2 <1>) 
replace `r' = l1.`r'_wma
	}
label var placebo "Non-populist"
label var atakeover "Populist"
label var dmig "Growth rate"
label var wdmig "World growth"
...
local rhsall bankcrisis currcrisis debtcrisis dmig wdmig institutions war gini unemployrate conflicts advanced pop_jul global medial medial inflation debtgdp
...
local base bankcrisis dmig wdmig institutions inflation war advanced
local rhs1  `base'
local rhs2  `base' currcrisis debtcrisis
local rhs3  `base' debtgdp
local rhs4  `base' conflicts
local rhs5  `base' gini
local rhs6  `base' global
local rhs7  `base' medial
local rhs8  `base' currcrisis debtcrisis debtgdp conflicts gini medial global
local rhs9  `base' unemployrate			
...
esttab mar* using ${tables}/TableC1_mig.tex, drop(dum*) cell(b(fmt(3) star) se(fmt(3) par)) noobs stats() varwidth(32) eqlabels(none) mlabels(none) starlevels(* 0.1 ** 0.05 *** 0.01) label nonotes replace page prehead("\begin{tabular}{l*{9}{c}}") postfoot("")

esttab mco* using ${tables}/TableC1_mig.tex, cell(b(fmt(3) star) se(fmt(3) par)) noobs stats() varwidth(32) starlevels(* 0.1 ** 0.05 *** 0.01) nonumber label nonotes eqlabels(none) mlabels(none) append page postfoot("") prehead("")  

esttab _*  using ${tables}/TableC1_mig.tex, drop(*) stats(Observations AUC AUC_se r2_p, fmt(%4.3fc)) modelwidth(9) varwidth(32) label nonotes nonumber eqlabels(none) mlabels(none) append page prehead("") postfoot("\end{tabular}")
...
cap save ${data_clean}/probabilities_mig, replace
restore 
...
order No
*Table C2
texsave * using ${tables}/TableC2_mig.tex, title(TABLE C2) replace
restore
...
reg mig_ch1 atakeover placebo `rhs1' d1-d`g' phat1 if _1_sample==1, noconstant cluster(iso) 	
...
gen asample = 1 if e(sample)==1
	...
	forvalues i=1/15 {
		reg mig_ch`i' placebo atakeover `rhs1'  d1-d`g' [pweight=invwt] if asample==1, noconstant cluster(iso)
...
twoway (rarea upbplacebo0 dnbplacebo0 Years, fcolor(gs12) lcolor(white) lpattern(solid)) ///
       (line bplacebo0 Years, lcolor(blue) lpattern(solid) lwidth(thick)) ///
       (line batakeover0 Years, lcolor(red) lpattern(shortdash) lwidth(vthick)) ///
       (line zero Years, lcolor(black)) if Year <=15, ///
       yscale(range(-2 2)) ///
       ylabel(-2(1)2, nogrid labsize(medlarge) angle(0) nogrid) ///
       ytitle("Gap in Net Migration Rate (per 1,000 population)", size(medsmall) margin(small)) ///
       xlabel(, labsize(medlarge) nogrid) ///
       graphregion(color(white)) plotregion(color(white)) ///
       subtitle("", size(small)) ///
       title("Stage 2: Inverse-propensity weighted local projection", color(black) size(large) margin(medium)) ///
       legend(off) xsize(3) ysize(3) nodraw name(stage2, replace)

*Figure 5
gr combine stage1 stage2, rows(1) iscale(0.75) graphregion(color(white) margin(l=0 r=3 t=1)) imargin(0 0) xsize(10)  ysize(4.25) ///
title("Net Migration after Populist Comes to Power: IPWRA-LPs" , ///
          size(large) margin(medium))
		  
gr export ${figures}/Figure5_mig.pdf, replace

*Table C3
esttab IPW* using ${tables}/TableC3_mig.tex, replace scalars(NonpPop_pdiff) se r2 keep(atakeover placebo) nonum compress b(2) se(2) sfmt(2) obslast nonotes label starlevels(* 0.1 ** 0.05 *** 0.01)
