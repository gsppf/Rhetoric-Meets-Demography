...
// DESCRIPTION:       LP-IV for old-age dependency ratio 
//      		      analysis following Funke et al. methodology
...
// 1. Create log old age depednency and forward growth rates (dependent variables)
gen lold  = log(oldage_ratio)
gen dlold = 100 * D.lold

forvalues h = 1/15 {
	gen dlold_fut`h' = (f`h'.lold - lold) * 100
	label var dlold_fut`h' "65+/14-64 Y+`h'"
}

// 2. Create baseline fertility growth and cross-year mean
egen mgrlold = mean(dlold), by(year)
...
// 1. log shares of the population  
gen lyoung   = log(share_young)   if share_young   > 0  
gen lwork = log(share_working) if share_working > 0  
gen lelderly = log(share_elderly) if share_elderly > 0  

// 2. log fertility rate  
gen lfert     = log(fertility)     if fertility     > 0  
...
twoway (rarea up_np lo_np years, fcolor(gs12) lcolor(white) lpattern(solid)) ///
	   ...
	   legend(rows(1) position(6) ring(2) label(3 "All populists") ///
	   label(2 "Trend in other years") label(1 "95% CI") order(3 2 1) ///
	   symxsize(*0.375) symysize(*0.375) size(medlarge) region(lwidth(none))) ///
	   xlabel(, labsize(large) nogrid) ylabel(, labsize(medlarge) angle(0) nogrid) ///
	   title("Panel A: Old Age Dependency Ratio Projected Trends" , color(black) size(large) margin(medium)) ///
	   ytitle("Percent (100 × log)", size(medsmall) margin(small)) xtitle("", size(medsmall)) ///
	   graphregion(color(white)) plotregion(color(white)) subtitle("", size(small)) ///
...
twoway (line irf_ap_gap years, lcolor(black) lpattern(solid) lwidth(thick)) ///
	   ...
	   title("Panel B: Old Age Dependency Ratio Projected Gaps", color(black) size(large) margin(medium)) ///
...
gr combine pl pr, rows(1) iscale(0.75) ///
	...
	title("Old Age Dependency Paths after Populist Governments Enter into Office: Local Projections" , ///
          size(medlarge) margin(medium))
		  
graph export ${figures}/Figure4_oldage.pdf, replace

// 15. Table C4
esttab LP* using "${tables}/TableC4_oldage.tex", replace ///
