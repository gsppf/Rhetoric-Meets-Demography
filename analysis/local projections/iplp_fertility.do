...
//	MODIFIED: 		
//	DESCRIPTION:	This file generaets IPWRA-LP for fertility
...
gen dlfert  = 100 * d.lfert

forvalues i = 1/15 {
	gen lfert`i' = 100 * (f`i'.lfert - lfert)	
	label var lfert`i' "Logged Fertility `i'"
}

egen wdlfert = mean(dlfert), by(year)
...
foreach r in bankcrisis currcrisis debtcrisis dlfert wdlfert institutions war gini unemployrate conflicts female migration birth_age inflation debtgdp lrgdppc mgrlrgdppc {
qui tssmooth ma `r'_wma = `r',  weights(5 4 3 2 <1>) 
replace `r' = l1.`r'_wma
	}
label var placebo "Non-populist"
label var atakeover "Populist"
label var dlfert "Growth rate"
label var wdlfert "World growth"
...
local rhsall bankcrisis currcrisis debtcrisis dlfert wdlfert institutions war gini unemployrate conflicts female migration birth_age inflation debtgdp lrgdppc mgrlrgdppc
...
local base bankcrisis dlfert wdlfert institutions inflation war migration
local rhs1  `base'
...
local rhs6  `base' female
local rhs7  `base' birth_age
local rhs8  `base' currcrisis debtcrisis debtgdp conflicts gini female birth_age
local rhs9  `base' unemployrate mgrlrgdppc lrgdppc
...
esttab mar* using ${tables}/TableC1_fert.tex, drop(dum*) cell(b(fmt(3) star) se(fmt(3) par)) noobs stats() varwidth(32) eqlabels(none) mlabels(none) starlevels(* 0.1 ** 0.05 *** 0.01) label nonotes replace page prehead("\begin{tabular}{l*{9}{c}}") postfoot("")

esttab mco* using ${tables}/TableC1_fert.tex, cell(b(fmt(3) star) se(fmt(3) par)) noobs stats() varwidth(32) starlevels(* 0.1 ** 0.05 *** 0.01) nonumber label nonotes eqlabels(none) mlabels(none) append page postfoot("") prehead("")  

esttab _*  using ${tables}/TableC1_fert.tex, drop(*) stats(Observations AUC AUC_se r2_p, fmt(%4.3fc)) modelwidth(9) varwidth(32) label nonotes nonumber eqlabels(none) mlabels(none) append page prehead("") postfoot("\end{tabular}")
...
title("") ylabel(, labsize(medlarge) angle(0) nogrid) xlabel(0(1)1, labsize(medlarge) nogrid) ytitle("Frequency") xtitle("Estimated probability of treatment") graphregion(color(white)) ///
...
cap save ${data_clean}/probabilities_fert, replace
restore 
...
order No
*Table C2
texsave * using ${tables}/TableC2_fert.tex, title(TABLE C2) replace
restore
...
reg lfert1 atakeover placebo `rhs1' d1-d`g' phat1 if _1_sample==1, noconstant cluster(iso) 	
...
gen asample = 1 if e(sample)==1
	...
	forvalues i=1/15 {
		reg lfert`i' placebo atakeover `rhs1'  d1-d`g' [pweight=invwt] if asample==1, noconstant cluster(iso)
...
title("Fertility after Populist Comes to Power: IPWRA-LPs" , ///
          size(large) margin(medium))


gr export ${figures}/Figure5_fert.pdf, replace

*Table C3
esttab IPW* using ${tables}/TableC3_fert.tex, replace scalars(NonpPop_pdiff) se r2 keep(atakeover placebo) nonum compress b(2) se(2) sfmt(2) obslast nonotes label starlevels(* 0.1 ** 0.05 *** 0.01)
